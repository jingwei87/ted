...
CDCodec::CDCodec(int CDType, int n, int m, int r, CryptoPrimitive* cryptoObj)
{
    ...
    if ((CDType_ == AONT_RS_TYPE) || (CDType_ == OLD_CAONT_RS_TYPE) || (CDType_ == CAONT_RS_TYPE)) { /*CDCodec based on AONT-RS, old CAONT-RS, or CAONT-RS*/
        if (n <= 0) {
            fprintf(stderr, "Error: n should be > 0!\n");
            exit(1);
        }
 ...
 */
bool CDCodec::crsssEncoding(unsigned char* secretBuffer, int secretSize, unsigned char* shareBuffer, int* shareSize, unsigned char* key)
{
...
bool CDCodec::aontRSEncoding(unsigned char* secretBuffer, int secretSize,
    unsigned char* shareBuffer, int* shareSize, unsigned char* key)
{
 ...
 * @return - a boolean value that indicates if the encoding succeeds
 */
bool CDCodec::encoding(unsigned char* secretBuffer, int secretSize, unsigned char* shareBuffer, int* shareSize, unsigned char* key)
{
    bool success = 0;

    if (CDType_ == CRSSS_TYPE) { /*CDCodec based on CRSSS*/
        success = crsssEncoding(secretBuffer, secretSize, shareBuffer, shareSize, key);
    }

    if (CDType_ == AONT_RS_TYPE) { /*CDCodec based on AONT-RS*/
        success = aontRSEncoding(secretBuffer, secretSize, shareBuffer, shareSize, key);
    }

    if (CDType_ == OLD_CAONT_RS_TYPE) { /*CDCodec based on old CAONT-RS*/
        success = caontRSOldEncoding(secretBuffer, secretSize, shareBuffer, shareSize, key);
    }

    if (CDType_ == CAONT_RS_TYPE) { /*CDCodec based on CAONT-RS*/
        success = caontRSEncoding(secretBuffer, secretSize, shareBuffer, shareSize, key);
    }
