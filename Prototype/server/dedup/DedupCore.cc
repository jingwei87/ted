 ...
 */
int DedupCore::restoreShareFile(const int &userID, const std::string &fullFileName, const int &versionNumber, 
	...
	if (inodeStat.ok()) {	
			...
			if (shareStat.ok()) {
				...
				/*if the share container is not cached in shareContainerCache*/
					...
					if (numOfCachedShareContainers < NUM_OF_CACHED_CONTAINERS) {
						...
						for (int k = numOfCachedShareContainers; k > 0; k--) {
							shareContainerCacheIndex[k] = shareContainerCacheIndex[k - 1];
						}
						...
						for (int k = NUM_OF_CACHED_CONTAINERS - 1; k > 0; k--) {
							shareContainerCacheIndex[k] = shareContainerCacheIndex[k - 1];
						}
						shareContainerCacheIndex[0] = tmp;
					}
				...
				if (shareFileBufferOffset + shareEntrySize_ + pShareIndexValueHead->shareSize > sentShareFileBufferSize) {
					...
					indicator = -5;
					sentDataSize = shareFileBufferOffset - sentMsgHeadSize;
					memcpy(shareFileBuffer, (char*)&indicator, sizeof(int));
					memcpy(shareFileBuffer + sizeof(int), (char*)&sentDataSize, sizeof(int));
		...
		if (shareFileBufferOffset > sentMsgHeadSize) {
			...
			cout<<"second the total size need to send "<<sentDataSize<<endl;
			int totalSendSize = 0;
			
			while (totalSendSize < sentDataSize) {
				int byteCount = -1;
				if ( (byteCount = send(socketFD, shareFileBuffer + totalSendSize, SOCKET_BUFFER_SIZE, 0)) == -1) {
					fprintf(stderr, "Error receiving data %d\n", errno);
				}
				totalSendSize += byteCount;
			}
